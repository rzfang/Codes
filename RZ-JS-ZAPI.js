...
function Z_DOM_Extend ()
{
  ...
  Element.prototype.Find = function (SltrStr)
    {
      if (typeof SltrStr !== 'string' || SltrStr.length === 0)
      { return []; }
  ...
  Element.prototype.Above = function (Lv)
    {
      ...
      for (var i = 1; (i < Lv) && (PrtNd !== document); i++)
      { PrtNd = PrtNd.parentNode; }
  ...
  Element.prototype.AddEvent = function (EvtStr, Fctn)
    {
      if (!EvtStr || typeof EvtStr !== 'string' || !Fctn || typeof Fctn !== 'function')
      { return null; }
  ...
  window.Z = function (ES)
  {
    var Tp = typeof ES,
        NdA = null; // 'NdA' = Node Array.

    if (Tp === 'object' && ES.tagName !== 'undefined' && ES.nodeType === 1)
    { NdA = [ES]; }
    else if (Tp === 'string')
    { NdA = document.querySelectorAll(ES).ToArray(); }
    else
    { NdA = null; }
  ...
  window.Z.NewNode = function (TgNm) { return document.createElement(TgNm); };
}
Z_DOM_Extend();
